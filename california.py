# -*- coding: utf-8 -*-
"""
Modified for California Housing Dataset (Regression)
...
CONFIG = {
    ...
    "TRIALS": 100,
    "REPETITIONS": 10,
    "TARGET_METRIC_NAME": "mse_loss", 
}
print("California Housing Dynamic PARAMS (Added High-Performance XLA Loop)")
...
# --------------------------
# For the 3 hyperparameter experiments, batch size, learning rate and epochs were used.
...
# Load Base dataset
# --------------------------
print("Loading California Housing Dataset into memory...")
california = fetch_california_housing()

X_raw = california.data.astype(np.float32)
y_raw = california.target.reshape(-1, 1).astype(np.float32)
...
def build_model(input_dim, learning_rate, activation, optimizer_name, dropout_rate, weight_decay, rep_seed):
    ...
    model = keras.Sequential([
        ...
        keras.layers.Dense(32, activation=activation, kernel_regularizer=l2_reg, kernel_initializer=kernel_init),
        keras.layers.Dropout(dropout_rate, seed=rep_seed),
        keras.layers.Dense(1, activation='linear', kernel_initializer=kernel_init)     
    ])
...
if __name__ == "__main__":
    ...
    for method_name, setup in RUN_CONFIG.items():
        ...
        for t in range(CONFIG["REPETITIONS"]):
            ...
            X_train_g = scaler.fit_transform(X_tr_raw).astype(np.float32)
            X_test_g = scaler.transform(X_te_raw).astype(np.float32)
            y_train_g = y_tr_raw
            y_test_g = y_te_raw
    ...
    print("\n===== FINAL RESULTS ACROSS ALL REPETITIONS (Validation MSE Loss) =====")
    for method_name, scores in results.items():
        calculate_stats(scores, method_name)
    
    # if ALL_METHODS_HISTORY:
    #     save_pbt_data(ALL_METHODS_HISTORY, filename="california_results.pkl")